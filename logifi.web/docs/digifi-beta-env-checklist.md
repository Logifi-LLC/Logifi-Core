...
|----------|------------------|-------|
| `DIGIFI_MODEL` / `NUXT_DIGIFI_MODEL` | `gemini-3.8-flash` | Weaker or legacy models reduce OCR quality. Use `gemini-3.5-flash` or `claude-sonnet-4-6` for A/B tests (Claude requires `ANTHROPIC_API_KEY`). Retired ids like `claude-3-5-sonnet-20241022` are auto-mapped to `claude-sonnet-4-6`. |
