<template>
  <div class="space-y-3">
    ...
    <p :class="['px-1 text-xs font-quicksand mb-2', isDarkMode ? 'text-gray-400' : 'text-gray-500']">
      <template v-if="isIos">
        Choose which details appear on each logbook entry. Date, route, aircraft, and total always show in the card header. Flight number shows in the header when enabled. Pilots show as a line above the footer when enabled.
      </template>
      <template v-else>
