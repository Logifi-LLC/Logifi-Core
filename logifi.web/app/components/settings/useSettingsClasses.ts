...
export function useSettingsClasses(isDarkMode: ComputedRef<boolean> | boolean) {
  ...
  const input = computed(() =>
    [
      'w-full rounded-lg border px-3 py-2 text-sm font-quicksand focus:outline-none focus:ring-2 transition-colors',
      dark.value
  ...
  return { section, input, readOnlyField, label, helper, sectionTitle, btnPrimary, btnSecondary, dark }
}
